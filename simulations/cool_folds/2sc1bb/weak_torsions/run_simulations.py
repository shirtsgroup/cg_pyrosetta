...
def run_mc_simulation(job):
    ...
    annealer_params = cg_pyrosetta.CG_monte_carlo.\
        CGMonteCarloAnnealerParameters(n_inner = 1200,
                                       t_init = 10,
                                       anneal_rate = 0.9,
                                       n_anneals = 50,
    ...
    # Pose to be folded
    pose = cg_pyrosetta.pyrosetta.pose_from_sequence("X[CG12x3:CGLower]X[CG12x3]X[CG12x3]X[CG12x3]X[CG12x3]X[CG12x3:CGUpper]")
    change_lengths = cg_pyrosetta.CG_movers.setBondLengths(pose, {"SC1 SC2":job.sp.sc_size, "SC3 SC4":job.sp.sc_size, "SC5 SC6":job.sp.sc_size})
