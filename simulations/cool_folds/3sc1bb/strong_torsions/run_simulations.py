...
def run_mc_simulation(job):
    ...
    # Pose to be folded
    pose = cg_pyrosetta.pyrosetta.pose_from_sequence("X[CG13x3:CGLower]X[CG13x3]X[CG13x3]X[CG13x3]X[CG13x3]X[CG13x3:CGUpper]")
    change_lengths = cg_pyrosetta.CG_movers.setBondLengths(pose, {"SC2 SC3":job.sp.sc_size, "SC5 SC6":job.sp.sc_size, "SC8 SC9":job.sp.sc_size})
