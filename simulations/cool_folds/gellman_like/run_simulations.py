...
def set_parameters(job):
    ...
    cg_pyrosetta.change_parameters.changeAngleParameters(
        {
         'CG1 CG2 CG3' : [800, job.sp.bond_angle], #[k_theta, angle (degrees)]
        },
...
def run_mc_simulation(job):
    os.chdir(job.ws)
    cg_pyrosetta.init(extra_res_fa="/ocean/projects/cts160011p/tfobe/foldamers/cg_pyrosetta/cg_pyrosetta/data/residue_type_sets/CG30.params")
    ...
    # Pose to be folded
    pose = cg_pyrosetta.pyrosetta.pose_from_sequence("X[CG30:CGLower]X[CG30]X[CG30]X[CG30]X[CG30]X[CG30]X[CG30]X[CG30]X[CG30]X[CG30]X[CG30]X[CG30]X[CG30]X[CG30:CGUpper]")

    # Set bond lenghts of pose to specified BLs
    bl_mover = cg_pyrosetta.CG_movers.setBondLengths(pose, {"BB1 BB2":job.sp.bond_length, "BB2 BB3":job.sp.bond_length, "BB3 BB1":job.sp.bond_length})
    bl_mover.apply(pose)
