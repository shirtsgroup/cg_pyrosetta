...
def main():
    ...
    for sc in sc_sizes:
        ...
        for job in project.find_jobs({'sc_size':sc, 'eps':target_eps}):
            ...
            csv_entry.append(np.max(z_score_matrix[mean_cluster_energies.index(min_energy), :]))
            ...
            csv_entry.append(np.mean(min_energy_intermedoid_rmsds[min_energy_intermedoid_rmsds != 0]))
            noise_cluster = False
