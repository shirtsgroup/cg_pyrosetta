...
def get_energy_trajectory(job):
    ...
    ax2.set_ylabel("Simulated Temperature", color = 'r', fontsize=20)
    plt.rcParams.update({'font.size' : 22})
    for traj in energy_traj:
        ax1.plot(energies.values[:,0], traj, alpha = 0.4, lw=2)
        ax1.set_xlabel("Steps", fontsize=20)
        ax1.set_ylabel("Energy (A.E.U)", fontsize=20)
        ax1.set_title("\theta_{BB} = " + str(round(job.sp['sc_size'],4)), fontsize=20)
...
def get_cluster_silhouette_scores(job):
    ...
    if len(cluster_sizes) > 1:
        ...
        print("Calculated:", np.mean(silhouette_scores[np.where(new_labels != -1)]))
        print("Actual:", silhouette_avg)
        ...
        print(avg_cluster_silhouette_scores)
...
@FlowProject.pre(has_clusters)
def non_parametric_rmsd_stats(job):
    with H5Store(job.fn('clustering.h5')).open(mode='r') as data:
            labels = data['labels'][:]
    if len(cluster_sizes) > 0:
        clusters = list(np.unique(labels))
        for i in clusters:
            ...
            if i == -1:
                i_label = "noise"
            min_structure = md.load(job.fn("cluster_output/cluster_" + i_label + "_minimum.pdb"))
            cluster = md.load(job.fn("cluster_output/cluster_" + i_label + ".pdb"))
            rmsd_values = md.rmsd(cluster, min_structure)
...
@flow.directives(fork=False)
@FlowProject.pre.after(run_clustering)
@FlowProject.post.isfile("helix_fitting/helix_fit_cluster_0_minimum.pdf")
