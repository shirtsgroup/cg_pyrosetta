...
def get_energy_trajectory(job):
    ...
    for energy, temp in zip(energy_traj, temp_traj):
        ax1.scatter(1/temp, energy, alpha = 0.4, s=10)
        ax1.set_xlabel("1/kT (A.E.U)")
        ax1.set_ylabel("Energy (A.E.U)")
        ax1.set_title("Bond Angle = " + str(round(job.sp['bond_angle'],4)))
        ax1.set_xscale('log')
...
def run_clustering(job):
    ...
    # Run clustering and store output in job.dataFoldamers offer a more diverse design space compared to traditional biopolymers
...
def cluster_energy_rmsd_scatter(job):
    ...
    energy_stdevs = []
    for i in clusters:
        medoid_mdtraj = md.load(job.fn("cluster_output/medoid_" + str(i) + ".pdb"))
        cluster_indices = np.where(labels == i)[0]
...
def energy_z_score_matrix(job):
    ...
    for i in clusters:
        for j in clusters:
            if i != j:
                z_score_matrix[i,j] = np.abs(compute_2_population_z_score(all_cluster_energies[i], all_cluster_energies[j]))
...
# @FlowProject.operation
# def cluster_internal_coordinate_distributions(job):
#    pass
