...
def main():
    ...
    sc_sizes.sort()
    csv_table = []
    csv_table.append(["bond_angle", "eps", "n_clusters", "avg_silhouette_score", "min_z_score*", "minimum_energy", "min_energy_cluster_rmsd", "min_energy_avg_intermedoid_rmsd", "noise_cluster", "rmse_helix", "rmse_cyl", "min_medoid"])
    for sc in sc_sizes:
        print("BA:", sc)
        silhouette_scores = []
        eps_values = []
        ...
        if len(silhouette_scores) != 0:
            ...
            print(n_clusters)
            cluster_metric = silhouette_scores / n_clusters
            print(cluster_metric)

            target_eps = eps_values[cluster_metric.tolist().index(np.max(cluster_metric))]
            ...
            for job in project.find_jobs({'bond_angle':sc, 'eps':target_eps}):
                ...
                print(min_energy_intermedoid_rmsds[np.invert(np.isclose(min_energy_intermedoid_rmsds, 0))])
